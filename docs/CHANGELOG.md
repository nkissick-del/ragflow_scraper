...
- `docs/reference/METADATA_SCHEMA.md`: Aligned required fields with `validate_metadata()`/`prepare_metadata_for_ragflow()` (organization, source_url, scraped_at, document_type), updated flattening rules to match implementation, and refreshed PDF/Article payload examples.
...
- Fixed **new** method to avoid conflated class/instance state (2026-01-07)
