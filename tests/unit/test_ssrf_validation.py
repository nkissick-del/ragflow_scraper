...
def _get_validate_fn():
    ...
    with patch("app.utils.get_logger", return_value=MagicMock()):
        from app.web.blueprints.settings import _validate_url_ssrf
...
class TestSSRFValidation:
    ...
    def test_blocked_link_local_ip(self):
        ...
        with patch("app.web.blueprints.settings.socket.getaddrinfo") as mock_dns:
    ...
    def test_dns_failure_rejects(self):
        ...
        validate = _get_validate_fn()
        with patch("app.web.blueprints.settings.socket.getaddrinfo") as mock_dns, \
             patch("app.web.blueprints.settings.log_event"):
